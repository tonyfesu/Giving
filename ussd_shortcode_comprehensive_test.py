...
def test_get_causes_ussd_shortcode():
    ...
    if missing_ussd:
        print(f"Found {len(missing_ussd)} causes missing USSD shortcodes.")
        for cause_id in missing_ussd:
            print(f"  - Cause ID: {cause_id}")
    else:
        print(f"All {len(causes)} causes have USSD shortcodes.")
    
    # Verify all causes have cause_code fields
    for cause in causes:
    ...
    print(f"Verified all cause codes are unique")
    return True
...
def test_ussd_format_validation():
    ...
    # Verify USSD format for all causes with shortcodes
    valid_ussd_count = 0
    for cause in causes:
        if not cause.get("ussd_shortcode"):
            continue
...
def test_demo_causes_ussd():
    """Test that demo causes have USSD shortcodes"""
    response = requests.get(f"{API_URL}/causes")
    assert response.status_code == 200, f"Expected status code 200, got {response.status_code}"
    ...
    for name in demo_cause_names:
        ...
        for cause in causes:
            if cause["name"] == name:
                ...
                # If ussd_shortcode is empty, report it
                if not cause.get("ussd_shortcode"):
                    print(f"WARNING: Demo cause {name} has empty 'ussd_shortcode' field")
                    continue
...
if __name__ == "__main__":
    ...
    run_test("Demo Causes USSD", test_demo_causes_ussd)
    ...
    if test_results["failed"] > 0:
        ...
        print("RECOMMENDATIONS:")
        print("="*80)
        print("1. Update all existing causes to have valid USSD shortcodes")
        print("   - Create a database migration script to set ussd_shortcode for all causes")
        print("   - Format should be: *123*86*{cause_code}*[Amount]#")
        print("2. Ensure all new causes automatically get USSD shortcodes")
        print("   - Verify the create_cause function sets ussd_shortcode correctly")
    else:
        print("\n" + "="*80)
        print("USSD SHORTCODE IMPLEMENTATION STATUS:")
        print("="*80)
        print("✅ All tests passed! The USSD shortcode implementation is working correctly.")
        print("✅ New causes automatically get USSD shortcodes in the correct format.")
        print("✅ API responses include the cause_code and ussd_shortcode fields.")
        print("✅ Cause codes are unique 6-character alphanumeric strings.")
        print("✅ USSD shortcodes follow the format: *123*86*{cause_code}*[Amount]#")